...
function App() {
  ...
  React.useEffect(() => {
    async function fetchData() {
      ...
      const cartResponse = await axios.get(
        "https://63c590a5f3a73b347859655a.mockapi.io/cart"
      );
      ...
      const itemsResponse = await axios.get(
        "https://63c590a5f3a73b347859655a.mockapi.io/items"
      );
  ...
  const onAddToCart = (obj) => {
    if (cartItems.find((item) => item.id === obj.id)) {
      // Numbers если убрать, то товар исчезает, но в бэке остаётся
      axios.delete(
        `https://63c590a5f3a73b347859655a.mockapi.io/cart/${obj.id}`
      );
