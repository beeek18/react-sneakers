...
function App() {
  ...
  React.useEffect(() => {
    async function fetchData() {
      ...
      const favoritesResponse = await axios.get('https://63ca86f9f36cbbdfc75a58d4.mockapi.io/favorites');
  ...
  const onAddToFavorite = async (obj) => {
    try {
      if (favorites.find((favObj) => Number(favObj.id) === Number(obj.id))) {
        axios.delete(`https://63ca86f9f36cbbdfc75a58d4.mockapi.io/favorites/${obj.id}`);
        setFavorites((prev) => prev.filter((item) => Number(item.id) !== Number(obj.id)));
      } else {
        const { data } = await axios.post('https://63ca86f9f36cbbdfc75a58d4.mockapi.io/favorites', obj);
